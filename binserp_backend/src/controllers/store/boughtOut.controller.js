...
export const updateBoughtOut = async (req, res) => {
  try {
    ...
    if (isDeactivating) {
      const currentDoc = await BoughtOut.findOne({ _id: id, company: companyId }).lean()
    ...
    try {
      ...
      console.warn("Inventory sync error on updateBoughtOut:", invErr.message);
    }
...
export const deleteBoughtOut = async (req, res) => {
  try {
    ...
    const existing = await BoughtOut.findOne({ _id: id, company: companyId }).lean()
      || await RmBoItem.findOne({ _id: id, company: companyId }).lean();
    if (!existing) {
      return res.status(404).json({ message: "Bought Out Item not found" });
    }
