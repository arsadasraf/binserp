...
import PPCWorkstationTab from "../../../components/PPCWorkstationTab";
...
export default function PPCWorkstationMasterPage() {
  const { setHeader } = useHeader();
  const searchParams = useSearchParams();
  const router = useRouter();
  const createForMachine = searchParams.get("createForMachine") || undefined;

  useEffect(() => {
    setHeader("Shop Floor Workstations", "Manage shop floor workstations and departments grouping multiple machines and assigned processes.");
  }, [setHeader]);
      ...
      <div className="p-4 max-w-[1600px] mx-auto">
        ...
        <div className="mt-4 space-y-4 animate-in fade-in slide-in-from-bottom-4 duration-500">
          ...
          <div className="bg-white dark:bg-gray-900 rounded-2xl shadow-sm border border-gray-100 dark:border-gray-800 p-4 sm:p-6">
            <PPCWorkstationTab
              preselectedMachineId={createForMachine}
              onModalClose={() => {
                if (createForMachine) {
