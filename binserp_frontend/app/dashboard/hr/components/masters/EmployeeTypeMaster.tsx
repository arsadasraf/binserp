...
import { EmployeeType } from "../../types/hr.types";
...
export default function EmployeeTypeMaster() {
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        try {
            ...
            const url = `${API_BASE_URL}/api/hr/employee-type`;
            ...
            alert("Failed to save employee type. Please try again.");
        } finally {
            setSubmitting(false);
        }
    };
    ...
    const handleDelete = async (id: string) => {
        ...
        try {
            const token = localStorage.getItem("token");
            await axios.delete(
                `${API_BASE_URL}/api/hr/employee-type/${id}`,
                {
                    headers: { Authorization: `Bearer ${token}` },
                }
            );
            ...
            <div className="bg-white border border-gray-100 dark:bg-slate-800 dark:border-slate-700 flex flex-row gap-3 items-center justify-between md:gap-4 p-4 rounded-xl shadow-sm">
                ...
                <div className="flex items-center gap-2">
                    <HrMasterExcelActions
                        masterTab="employee-type"
                        data={employeeTypes}
                        onSuccess={fetchEmployeeTypes}
                    />
                    <button
                        onClick={handleOpenAdd}
                        className="bg-blue-600 flex flex-none gap-2 hover:bg-blue-700 items-center justify-center md:px-4 px-3 py-2 rounded-lg text-white transition-colors text-sm font-bold shadow-sm"
                    >
                        <Plus size={18} />
                ...
                {loading ? (
                    ...
                    <>
                                ...
                                <tbody className="divide-gray-100 divide-y">
                                    {filteredEmployeeTypes.map((type) => (
                                        ...
                                        >
                                            ...
                                            <td className="dark:text-gray-300 px-6 py-4 text-gray-600">{type.description || "-"}</td>
                        ...
                        <div className="flex flex-col gap-3 md:hidden">
                            {filteredEmployeeTypes.map((type) => (
                                <div key={type._id} className="bg-white border border-gray-100 dark:bg-slate-800 dark:border-slate-700 flex flex-col gap-3 p-4 rounded-xl shadow-sm">
                                    <div className="flex items-start justify-between">
                                        <div>
                                            <h4 className="dark:text-white font-bold text-gray-900">{type.name}</h4>
                                            {type.description && (
                                                <p className="dark:text-gray-400 mt-0.5 text-gray-500 text-sm">{type.description}</p>
                                            )}
                                        </div>
                                    </div>
                    ...
                    <div className="animate-in bg-white dark:bg-slate-800 duration-200 fade-in max-w-md mx-4 p-6 rounded-xl shadow-xl w-full zoom-in-95">
                        <div className="flex items-center justify-between mb-4">
                            <h3 className="dark:text-white font-bold text-gray-900 text-lg">
                                {isEditing ? "Edit Employee Type" : "Add New Employee Type"}
                            </h3>
