...
export default function PurchaseVendorQuotationPage() {
  ...
  const { quotations, loading, refetch, handleQuotationSubmit, handleQuotationUpdate, handleDelete } = useStoreData("vendor-quotation", "vendor", token);
  ...
  return (
    <div className="space-y-4">
      <div className="flex justify-between items-center bg-white dark:bg-slate-900 p-4 rounded-xl shadow-sm border border-gray-100 dark:border-slate-800">
        <div>
          <h1 className="text-xl font-bold text-gray-900 dark:text-white">Vendor Quotations</h1>
          <p className="text-xs text-gray-500">Manage received vendor price quotes</p>
        </div>
        <button
