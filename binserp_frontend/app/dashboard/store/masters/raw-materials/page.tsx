...
const extractErrorMessage = (error: any, fallback = "Failed to save Raw Material."): string => {
...
export default function RawMaterialsPage() {
  ...
  const handleToggleStatus = async (item: any) => {
    ...
    try {
      ...
      setToast({ type: 'success', message: `Raw Material status updated to ${newStatus}` });
    } catch (err: any) {
  ...
  const handleDelete = async (id: string) => {
    ...
    if (confirm(`Are you sure you want to delete "${target?.name || 'this Raw Material'}"?`)) {
      try {
        ...
        setToast({ type: 'success', message: `Raw Material "${target?.name || 'Item'}" deleted successfully.` });
      } catch (error: any) {
        ...
        if (target && (errMsg.toLowerCase().includes("stock") || errMsg.toLowerCase().includes("active") || errMsg.toLowerCase().includes("transaction"))) {
          ...
          setToast({ type: 'error', message: `Error deleting Raw Material: ${errMsg}` });
        }
      }
    }
  };
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (editingItem) {
        ...
        setToast({ type: 'success', message: `Raw Material "${cleanName}" updated successfully!` });
      } else {
        await createRecord({ tab: "raw-material", body: submitData, isFormData: true }).unwrap();
        setToast({ type: 'success', message: `Raw Material "${cleanName}" created successfully!` });
      }
      ...
      const errMsg = extractErrorMessage(error, "Failed to save Raw Material.");
      console.warn("Save Raw Material notice:", errMsg);
      setFormError(errMsg);
    }
  };
      ...
      >
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          {formError && (
            <div className="flex items-start justify-between gap-3 p-3.5 bg-rose-50 dark:bg-rose-950/60 border border-rose-200 dark:border-rose-900/60 rounded-xl text-rose-800 dark:text-rose-200 text-sm shadow-xs animate-in fade-in slide-in-from-top-1 duration-200">
              <div className="flex items-start gap-2.5">
                ...
                <div>
                  <div className="font-semibold">Unable to Save Raw Material</div>
          ...
          <div className="flex justify-end gap-3 pt-4 border-t border-gray-100 dark:border-slate-800">
            ...
            >
              {isCreating || isUpdating ? "Saving..." : (editingItem ? "Update Raw Material" : "Save Raw Material")}
            </button>
          </div>
        </form>
      </Modal>
