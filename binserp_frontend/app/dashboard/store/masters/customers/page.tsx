...
const extractErrorMessage = (error: any, fallback = "Failed to save customer."): string => {
...
export default function CustomersPage() {
  ...
  const handleToggleStatus = async (item: any) => {
    ...
    try {
      ...
      setToast({ type: 'success', message: `Customer status updated to ${newStatus}` });
    } catch (err: any) {
      console.error("Failed to update customer status", err);
      setToast({ type: 'error', message: `Failed to update customer status: ${extractErrorMessage(err)}` });
    }
  };
  ...
  const handleDelete = async (id: string) => {
    ...
    if (confirm(`Are you sure you want to delete "${target?.name || 'this customer'}"?`)) {
      try {
        ...
        setToast({ type: 'success', message: `Customer "${target?.name || 'Item'}" deleted successfully.` });
      } catch (error: any) {
        const errMsg = extractErrorMessage(error, "Failed to delete customer");
        if (target && (errMsg.toLowerCase().includes("stock") || errMsg.toLowerCase().includes("active") || errMsg.toLowerCase().includes("transaction") || errMsg.toLowerCase().includes("order") || errMsg.toLowerCase().includes("dc") || errMsg.toLowerCase().includes("invoice"))) {
          ...
          setToast({ type: 'error', message: `Error deleting customer: ${errMsg}` });
        }
      }
    }
  };
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (editingItem) {
        ...
        setToast({ type: 'success', message: `Customer "${cleanName}" updated successfully!` });
      } else {
        await createRecord({ tab: "customer", body: payload }).unwrap();
        setToast({ type: 'success', message: `Customer "${cleanName}" created successfully!` });
      }
      ...
      const errMsg = extractErrorMessage(error, "Failed to save customer.");
      console.warn("Save customer notice:", errMsg);
      setFormError(errMsg);
    }
  };
      ...
      >
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          {formError && (
            <div className="flex items-start justify-between gap-3 p-3.5 bg-rose-50 dark:bg-rose-950/60 border border-rose-200 dark:border-rose-900/60 rounded-xl text-rose-800 dark:text-rose-200 text-sm shadow-xs animate-in fade-in slide-in-from-top-1 duration-200">
              <div className="flex items-start gap-2.5">
                ...
                <div>
                  <div className="font-semibold">Unable to Save Customer</div>
          ...
          <div className="flex justify-end gap-3 pt-4 border-t border-gray-100 dark:border-slate-800">
            ...
            >
              {isCreating || isUpdating ? "Saving..." : (editingItem ? "Update Customer" : "Save Customer")}
            </button>
          </div>
        </form>
      </Modal>
