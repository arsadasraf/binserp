...
const extractErrorMessage = (error: any, fallback = "Failed to save Bought Out Item."): string => {
...
export default function BoughtOutPage() {
  ...
  const handleToggleStatus = async (item: any) => {
    ...
    try {
      ...
      setToast({ type: 'success', message: `Bought Out item status updated to ${newStatus}` });
    } catch (err: any) {
  ...
  const handleDelete = async (id: string) => {
    ...
    if (confirm(`Are you sure you want to delete "${target?.name || 'this Bought Out item'}"?`)) {
      try {
        ...
        setToast({ type: 'success', message: `Bought Out "${target?.name || 'Item'}" deleted successfully.` });
      } catch (error: any) {
        ...
        if (target && (errMsg.toLowerCase().includes("stock") || errMsg.toLowerCase().includes("active") || errMsg.toLowerCase().includes("transaction"))) {
          ...
          setToast({ type: 'error', message: `Error deleting Bought Out Item: ${errMsg}` });
        }
      }
    }
  };
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (!cleanName) {
      setFormError("Bought Out Item Name is required.");
      return;
    }
    ...
    try {
      ...
      if (editingItem) {
        ...
        setToast({ type: 'success', message: `Bought Out Item "${cleanName}" updated successfully!` });
      } else {
        await createRecord({ tab: "bought-out", body: submitData, isFormData: true }).unwrap();
        setToast({ type: 'success', message: `Bought Out Item "${cleanName}" created successfully!` });
      }
      ...
      const errMsg = extractErrorMessage(error, "Failed to save Bought Out Item.");
      console.warn("Save Bought Out Item notice:", errMsg);
      setFormError(errMsg);
    }
  };
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          {formError && (
            <div className="flex items-start justify-between gap-3 p-3.5 bg-rose-50 dark:bg-rose-950/60 border border-rose-200 dark:border-rose-900/60 rounded-xl text-rose-800 dark:text-rose-200 text-sm shadow-xs animate-in fade-in slide-in-from-top-1 duration-200">
              <div className="flex items-start gap-2.5">
                ...
                <div>
                  <div className="font-semibold">Unable to Save Bought Out Item</div>
          ...
          <div className="flex justify-end gap-3 pt-4 border-t border-gray-100 dark:border-slate-800">
            ...
            <button
              ...
              className="px-5 py-2 text-xs sm:text-sm font-medium text-white bg-amber-600 rounded-lg hover:bg-amber-700 disabled:opacity-50 transition-colors shadow-xs"
            >
              {isCreating || isUpdating ? "Saving..." : (editingItem ? "Update Bought Out Item" : "Save Bought Out Item")}
            </button>
          </div>
        </form>
      </Modal>
