...
import { Layers, Search, Factory, ArrowUpRight, ArrowDownLeft, CheckCircle2, Clock, Eye, RefreshCw, Filter, Boxes } from 'lucide-react';
...
export default function WipInventoryTab({ token, companyInfo, onError, onSuccess }: WipInventoryTabProps) {
    ...
    const fetchWipInventory = async () => {
        ...
        try {
            setLoading(true);
            const res = await apiGet('/api/store/wip/inventory', token);
    ...
    return (
        <div className="space-y-6 animate-in fade-in duration-300">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
                ...
                {/* Card 2: Total Sent Outward Qty */}
                <div className="bg-white dark:bg-slate-900 p-5 rounded-2xl border border-slate-200 dark:border-slate-800 shadow-sm flex items-center justify-between">
                    <div>
                        <span className="text-xs font-bold text-amber-500 uppercase tracking-wider block">Total Dispatched Sent</span>
                        <h3 className="text-2xl font-black text-amber-600 dark:text-amber-400 mt-1">
                            {summary.totalSentQty.toLocaleString()}
                        </h3>
                    </div>
                ...
                <div className="bg-white dark:bg-slate-900 p-5 rounded-2xl border border-slate-200 dark:border-slate-800 shadow-sm flex items-center justify-between">
                    <div>
                        <span className="text-xs font-bold text-emerald-500 uppercase tracking-wider block">Total Returned Received</span>
                        <h3 className="text-2xl font-black text-emerald-600 dark:text-emerald-400 mt-1">
                            {summary.totalReceivedQty.toLocaleString()}
                        </h3>
                    </div>
            ...
            <div className="bg-white dark:bg-slate-900 p-4 rounded-2xl border border-slate-200 dark:border-slate-800 shadow-sm flex flex-col md:flex-row justify-between items-center gap-4">
                ...
                <div className="flex flex-wrap items-center gap-3 w-full md:w-auto">
                    ...
                    <div className="relative flex-1 md:w-64">
                        ...
                        <input
                            type="text"
                            placeholder="Search WIP item or supplier..."
                            value={searchTerm}
            ...
            {loading ? (
                ...
                <div className="text-center py-16 bg-white dark:bg-slate-900 rounded-2xl border border-dashed border-slate-200 dark:border-slate-800">
                    ...
                    <h3 className="text-base font-bold text-slate-800 dark:text-slate-200">No WIP Inventory Items Found</h3>
                    <p className="text-xs text-slate-500 mt-1">Dispatched Job Work items will automatically show up here as active Work In Progress.</p>
                </div>
            ) : (
                    ...
                    <div className="overflow-x-auto">
                        <table className="w-full text-sm text-left">
                            <thead className="bg-slate-100 dark:bg-slate-800 text-xs font-bold text-slate-600 dark:text-slate-400 uppercase tracking-wider border-b border-slate-200 dark:border-slate-700">
                                <tr>
                                    <th className="px-5 py-3.5">Sent Material</th>
                                    <th className="px-5 py-3.5">Expected Return Item</th>
                                    <th className="px-5 py-3.5">Subcontractor</th>
                                    <th className="px-5 py-3.5">Process</th>
                                    <th className="px-5 py-3.5 text-center">Sent Out</th>
                                ...
                                {filteredItems.map((item) => (
                                    <tr key={item.id} className="hover:bg-slate-50/70 dark:hover:bg-slate-800/50 transition-colors">
                                        <td className="px-5 py-4 font-bold text-slate-900 dark:text-white">
                                            {item.sentItemName}
                                            <span className="block text-[10px] text-slate-400 font-normal uppercase">Type: {item.itemType}</span>
                                        </td>

                                        <td className="px-5 py-4 font-bold text-indigo-600 dark:text-indigo-400">
                                            {item.receivedItemName}
                                        </td>
                                        ...
                                        <td className="px-5 py-4 text-center font-bold text-amber-600 dark:text-amber-400">
                                            {item.totalSentQty} <span className="text-xs font-normal text-slate-400">{item.unit}</span>
                                        </td>

                                        <td className="px-5 py-4 text-center font-bold text-emerald-600 dark:text-emerald-400">
                                            {item.totalReceivedQty} <span className="text-xs font-normal text-slate-400">{item.receivingUnit}</span>
                                        </td>
                                        ...
                                        <td className="px-5 py-4 text-center">
                                            <span className={`inline-block px-3 py-1 rounded-full text-xs font-black font-mono ${
                                                ...
                                                {item.pendingWipQty} {item.receivingUnit}
                                            </span>
                                        </td>
