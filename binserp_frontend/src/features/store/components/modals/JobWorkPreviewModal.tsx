                    ...
                    {(challan as any).receiveHistory && (challan as any).receiveHistory.length > 0 && (
                        <div className="bg-slate-50 dark:bg-slate-800/50 p-4 rounded-2xl border border-slate-200 dark:border-slate-700 space-y-2">
                            <span className="text-xs font-bold text-slate-600 dark:text-slate-300 uppercase tracking-wider flex items-center gap-1.5">
                                <Clock size={14} /> Return Receipt History Log
                            </span>
                            <div className="space-y-1.5 pt-1">
                                {(challan as any).receiveHistory.map((hist: any, i: number) => {
                                    const matchedItem = challan.items.find((it: any) => it._id === hist.itemId);
                                    return (
                                        <div key={i} className="flex items-center justify-between text-xs bg-white dark:bg-slate-900 p-2.5 rounded-xl border border-slate-200 dark:border-slate-800">
                                            <div className="flex items-center gap-2">
                                                <span className="w-2 h-2 rounded-full bg-emerald-500"></span>
                                                <span className="font-bold text-slate-800 dark:text-slate-200">{hist.quantity} units</span>
                                                <span className="text-slate-400">received ({matchedItem?.itemName || 'Material'})</span>
                                            </div>
                                            <span className="text-slate-400 font-mono">{new Date(hist.date).toLocaleString('en-GB')}</span>
                                        </div>
                                    );
                                })}
                            </div>
